...
import { adminBrandIcon, adminLogoutItem, adminNavItems } from '@/lib/adminNavigation'
      ...
      >
        <div className="flex h-full flex-col p-4">
          <div className="mb-4 flex items-center justify-between">
            <Link to="/admin/dashboard" className="flex items-center gap-2" onClick={onClose}>
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-accent-primary/30">
                <BrandIcon className="h-4 w-4 text-highlight" />
              </div>
              <span className="text-sm font-semibold">
                Lumen<span className="text-white/50">.</span>
                <span className="ml-1 text-[10px] font-normal text-white/40">Admin</span>
              </span>
            </Link>
          ...
          <div className="mt-4 shrink-0 space-y-2 border-t border-border/30 pt-4">
            ...
            >
              <adminLogoutItem.icon className="h-4 w-4" />
              {adminLogoutItem.label}
            </button>
          </div>
        </div>
      </aside>
