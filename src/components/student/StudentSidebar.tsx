...
import { studentLogoutItem, studentNavItems } from '@/lib/studentNavigation'
      ...
      >
        <div className="flex h-full min-h-0 flex-col p-4">
          <div className="mb-4 flex items-center justify-between">
            <Link to="/student/dashboard" className="flex items-center gap-2" onClick={onClose}>
          ...
          <nav className="sidebar-scroll flex-1 min-h-0 space-y-0.5 overflow-y-auto overflow-x-hidden">
            {studentNavItems.map((item) => {
              ...
              const isActive =
                ...
                (isCatalog && location.pathname.startsWith('/catalog')) ||
                (!isCatalog &&
          ...
          <div className="mt-4 shrink-0 space-y-2 border-t border-border/30 pt-4">
              ...
              <Link to="/student/profile" onClick={onClose}>
                <div className="mb-2 flex items-center gap-3 rounded-[12px] bg-bg-secondary/40 p-3 transition-colors hover:bg-bg-secondary/60">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[11px] text-white/45">Student</p>
                  </div>
                </div>
              </Link>
            )}
            ...
            >
              <studentLogoutItem.icon className="h-4 w-4" />
              {studentLogoutItem.label}
            </button>
          </div>
        </div>
      </aside>
